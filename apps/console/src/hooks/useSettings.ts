import type { SettingsResponse, SettingsUpdateRequest } from '@harness/http/types';
import { useMutation, useQuery, useQueryClient } from '@tanstack/react-query';
import { useCallback, useEffect, useRef, useState } from 'react';
import { api } from '../api/client.ts';

const QUERY_KEY = ['console-settings'] as const;
  ...
  const mutation = useMutation({
    ...
    onMutate: async (vars) => {
      ...
      if (vars.scope === 'global') {
        ...
        const cap = prev.capabilities[vars.scope];
        if (cap) {
          const inheritedFromGlobal = { ...cap.inheritedFromGlobal };
          for (const k of Object.keys(vars.settings)) {
            if (k in inheritedFromGlobal) {
              inheritedFromGlobal[k] = false;
            }
          }
          const nextValues = { ...cap.values, ...vars.settings };
