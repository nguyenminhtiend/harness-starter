...
function apiKeyIsSet(mask: unknown): boolean {
  if (!mask || typeof mask !== 'object' || !('set' in mask)) {
    return false;
  }
  return Boolean((mask as ApiKeyMask).set);
}
...
export function SettingsPanel({ activeTool }: SettingsPanelProps) {
  ...
  const capQuery = useQuery({
    queryKey: ['console-capability', activeTool],
    queryFn: () => api.getCapability(activeTool),
        ...
        {tab === 'tool' && properties ? (
          ...
          >
            <p style={{ fontSize: 'var(--text-sm)', color: 'var(--text-secondary)', margin: 0 }}>
              Settings for <strong>{activeToolDef.title}</strong> (from capability schema). Changes
              save automatically.
            </p>
        ...
        {tab === 'keys' ? (
          ...
          >
            ...
            {Object.keys(values)
              ...
              .map((key) => {
                const mask = values[key];
                const set = apiKeyIsSet(mask);
                return (
                  <div
                    key={key}
                    style={{
