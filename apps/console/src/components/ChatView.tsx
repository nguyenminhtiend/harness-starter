import type { ConversationMessage, SessionEvent } from '@harness/http/types';
import {
  type CSSProperties,
  type KeyboardEvent,
  memo,
...
export function ChatView({
  ...
  useEffect(() => {
    if (conversationId && conversationId !== loadedConvRef.current) {
      ...
      setInput('');
      void api.getConversationMessages(conversationId).then((rows) => {
        setMessages(rows.messages.map(serverMsgToChatMsg));
